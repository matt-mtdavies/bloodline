...
export function computePedigree(graph, focusId, { expandedUp, partnerChoice, orientation = 'vertical', bloodlineOnly = false } = {}) {
  ...
  // A person with more than one partnership only ever shows ONE at a time
  // here — the swap pip switches which. An earlier version of this also drew
  // every OTHER partnership with shared children as its own sibling pod
  // beside the primary one; real feedback on the live result was that two
  // same-person pods sitting apart with no visual link between them read as
  // two unrelated couples, not "one person, two relationships" — reverted
  // rather than fixed in place, since the fix itself was a bigger, separate
  // design question. See git history for the removed mechanism if revisited.
  ...
  // ── Focal children row: drawn cards one step below, honest per-side
  //    connectors — a child linked to only ONE displayed member hangs from
  ...
  buildChildrenRow(focal, focal.childRows);
