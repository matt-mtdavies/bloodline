...
export function drawLinks(
  ...
  const drawGroup = (grp) => {
    ...
    // never inside the shading. Derived from the capsule's real geometry
    // rather than assumed horizontal, so a couple caught mid-rearrangement at
    // an angle still emits its line from outside the fill.
 ...
 */
export function capsuleBottom(a, b, hw) {
  const mx = (a.x + b.x) / 2;
  const my = (a.y + b.y) / 2;
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
// glance without reading the layout. style.width is the width at the parent
// end and style.endWidth at the child end; a taper is drawn as ONE filled
// ribbon rather than a stack of progressively-thinner strokes, so a tapered
// edge still costs a single draw call per link the way the plain stroke did.
