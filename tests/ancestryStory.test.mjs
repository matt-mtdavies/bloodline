 ...
 * assembly: the patrilineal + matrilineal ascending chains, their
 * convergence at the subject's parents' marriage, privacy stopping a line
 * short, and the generation-readiness gate.
...
test('no parents on record — both lines empty, no convergence', () => {
  const g = buildGraph([person('alice', 'female')], []);
  const facts = buildAncestryFacts(g, 'alice');
  assert.deepEqual(facts.paternalLine, []);
  assert.deepEqual(facts.maternalLine, []);
  assert.equal(facts.convergence, null);
});

test('patrilineal line walks father -> father\'s father -> ..., oldest first', () => {
  const g = buildGraph(
    [
      person('alice', 'female'),
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.deepEqual(facts.paternalLine.map((a) => a.name), ['Great-granddad', 'Granddad', 'Dad']);
});

test('matrilineal line walks mother -> mother\'s mother -> ..., independent of the paternal line', () => {
  const g = buildGraph(
    [
      person('alice', 'female'),
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.deepEqual(facts.maternalLine.map((a) => a.name), ['Grandmum', 'Mum']);
  assert.deepEqual(facts.paternalLine, []);
});

test('a step-parent is not part of either line', () => {
  const g = buildGraph(
...
test('an adoptive parent IS part of the line (same convention as isBioAdopt elsewhere)', () => {
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.deepEqual(facts.paternalLine.map((a) => a.name), ['Dad']);
});
...
test('the line stops when no further same-gender bio/adoptive parent is recorded', () => {
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.equal(facts.paternalLine.length, 2);
});
...
test('a private ancestor stops the line right there and is excluded outright', () => {
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.deepEqual(facts.paternalLine.map((a) => a.name), ['Dad']);
  assert.ok(!facts.paternalLine.some((a) => a.name === 'Should never appear'));
});
...
test('a summary-visibility ancestor appears name-only and nothing behind them is knowable', () => {
  ...
  const facts = buildAncestryFacts(g, 'alice');
  assert.equal(facts.paternalLine.length, 1);
  assert.equal(facts.paternalLine[0].restricted, true);
  assert.equal(facts.paternalLine[0].born, undefined);
});
...
test('ancestryReady: below the threshold returns false, at/above it returns true', () => {
  const known = (n) => Array.from({ length: n }, (_, i) => ({ name: `a${i}`, restricted: false }));
  assert.equal(ancestryReady({ paternalLine: known(ANCESTRY_MIN_ANCESTORS - 1), maternalLine: [] }), false);
  assert.equal(ancestryReady({ paternalLine: known(2), maternalLine: known(1) }), true);
});

test('ancestryReady: a restricted (name-only) ancestor does not count toward the threshold', () => {
  const facts = {
    paternalLine: [{ name: 'a', restricted: true }, { name: 'b', restricted: true }, { name: 'c', restricted: true }],
    maternalLine: [],
  };
...
test('factsHash is re-exported from lib/keepsake.js — same algorithm, no drift', () => {
  const a = { subject: { name: 'Alice' }, paternalLine: [], maternalLine: [], convergence: null };
