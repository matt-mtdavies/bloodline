...
function nominatimResult(lat, lon, displayName = 'Somewhere') {
  return [{ lat: String(lat), lon: String(lon), display_name: displayName }];
}
...
await atest('geocodePlaces: two distinct places, both cache misses, both fetched and cached', async () => {
  ...
  assert.deepEqual(result['London, England'], { lat: 51.5, lon: -0.12 });
...
await atest('geocodePlaces: a place already cached as "ok" is never re-fetched', async () => {
  const { env } = makeFakeDB([{ place_key: 'cardiff, wales', display_name: 'Cardiff', lat: 51.48, lon: -3.18, status: 'ok' }]);
  let calls = 0;
...
await atest('geocodePlaces: two places normalizing to the same key are fetched exactly once', async () => {
  ...
  assert.deepEqual(result['Cardiff, Wales'], { lat: 51.48, lon: -3.18 });
  assert.deepEqual(result['  cardiff, wales  '], { lat: 51.48, lon: -3.18 }, 'the original (unnormalized) string is still a valid output key');
});
